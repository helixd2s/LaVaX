...
namespace dxv {
    ...
    VkDescriptorSetLayout_T::VkDescriptorSetLayout_T(const dxv::VkPhysicalDevice_T* device, HPTR(VkDescriptorSetLayoutCreateInfo) createInfo, UPTR(VkAllocationCallbacks) callbacks) {
        ...
        for (uint32_t i = 0; i < createInfo->bindingCount; i++) {
            ...
            this->ranges.back().Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, createInfo->pBindings[i].descriptorCount, 0, createInfo->pBindings[i].binding, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);
