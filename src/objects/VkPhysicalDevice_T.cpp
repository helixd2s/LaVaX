...
namespace lvx {
    ...
    void VkPhysicalDevice_T::GetPhysicalDeviceMemoryProperties(VkPhysicalDeviceMemoryProperties* pMemoryProperties) {
        ...
        for (auto& type : heapTypes) {   // 
            ...
            D3D12_HEAP_PROPERTIES propertiesDefault = device->GetCustomHeapProperties(nodeMask, type);
            
            // 
            if (propertiesDefault.MemoryPoolPreference == D3D12_MEMORY_POOL_L1) { 
                flags |= VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT; 
            };
            if (propertiesDefault.CPUPageProperty == D3D12_CPU_PAGE_PROPERTY_WRITE_COMBINE) {
                flags |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_CACHED_BIT;
            };
            if (propertiesDefault.CPUPageProperty == D3D12_CPU_PAGE_PROPERTY_WRITE_BACK) {
                flags |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
            };
            
            // TODO: Getting Heap Memory Size
            maker.addHeap(VkMemoryHeap{ .size = 1024 * 1024 * 1024 * 8u, .flags = 
                (propertiesDefault.MemoryPoolPreference==D3D12_MEMORY_POOL_L1?VK_MEMORY_HEAP_DEVICE_LOCAL_BIT:0u) 
            }).addType(flags);
        };
